...
class DataController extends Controller
{
	...
	public function pcommonContent(){
		...
		//produt categories
		$result['pcategories'] = $this->pcategories();
		$result['categories'] = $this->categories();
	...
	public function pcategories(){
		...
		$pcategories = DB::table('categories')
				...
				->select('categories.categories_id as pid',
					 'categories.categories_image as pimage',
					 'categories.categories_icon as picon',
					 'categories.sort_order as porder',				 
					 'categories.categories_slug as pslug',
					 'categories.parent_id',
					 'categories.group_id',
					 'categories_description.categories_name as pname'
					 )
		...
		foreach($pcategories as $pcategories_data){
			array_push($result,$pcategories_data);

			$p_id = $pcategories_data->pid;
			...
			$categories = DB::table('categories')
				...
				->where('categories_description.language_id','=', Session::get('language_id'))
				->where('group_id','1')
				->where('categories.parent_id','=', $p_id)
				->get();
			...
			foreach($categories as $categories_data){
				...
				foreach($sub_categories as $sub_categories_data){
					...
					array_push($result,$sub_categories_data);		
